...
public class SensorListManagerJSON implements SensorList {
    ...
    public List<SensorRecord> getSensors() {
        ...
        try {
            ...
            log.error("Unable to find application location : " + ex.getLocalizedMessage());
        }
        ...
        TypeReference<List<SensorRecord>> typeReference = new TypeReference<>() {};

        //InputStream inputStream = TypeReference.class.getResourceAsStream(sensorFileLocation.toString());
        InputStream inputStream;
        try {
            inputStream = new FileInputStream(sensorFileLocation);
        } catch (FileNotFoundException ex) {
            log.error("Sensor File not found at location");
